...
응답에는 요청 인원을 수용할 수 있는 객실만 실립니다 — 요청 인원(`adults + children`, 총원)이 객실
정원(`roomType.maxOccupancy`)을 넘는 상품은 정규화 단계에서 제외됩니다. 이는 오류가 아니라 **조용한
제외**로, 미매핑·미확정 제외와 같은 층위입니다. 공급사가 정원으로 걸러 준다는 보장이 계약에 없으므로,
정원 대조를 서버 정규화에서 한 번 더 보장합니다. 기준 정원은 카탈로그 스냅샷의 `maxOccupancy`(방 속성의
authoritative 원천)이며, 아동 연령별 규칙이 계약에 없어 총원(성인+아동)으로 대조합니다.
