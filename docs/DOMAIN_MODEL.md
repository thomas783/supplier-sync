...
classDiagram
    ...
    class Availability {
        +int availableRooms "0 = 확정 매진"
        +boolean isAvailable "파생값 — availableRooms >= 1"
    }
...
| 재고 | 날짜별 잔여 수를 전 기간 병목으로 판정한 결과 | `Availability` (확정 숫자 하나 — 미확정은 정규화에서 제외) | 저장하지 않음 |

숙소·객실의 비영속 모델은 매핑 엔티티의 **투영**입니다 — 검색 결과에 필요한 것은 내부 식별자와 표시
속성뿐이라, 정규화가 매핑에서 이 형태로 만들어 상품에 싣습니다. 공급사 코드는 여기에 없습니다(어댑터
경계 밖으로 나오지 않는 원칙). 표시 속성(객실명·`maxOccupancy` 등)은 **매핑 스냅샷 기준**이라, 동기화
주기와 검색 사이 공급사가 값을 바꾸면 최대 그 주기만큼 낡을 수 있습니다 — 정적 콘텐츠로 다뤄 매 검색마다
다시 받지 않는다는 결정의 귀결이며(정원은 예약 차단·정산에 쓰이지 않는 표시 메타데이터), 재고·요금 같은
동적 값은 이와 달리 실시간 응답에서 옵니다. API 응답의 상품 항목도 이 조합을 같은
묶음(`property`·`roomType`·`availability`·`price`)으로 비춥니다 — 다만 전용 DTO를 거치는 표현 계층의
투영이며 계약은 [API.md](API.md)가 기준입니다.
...
- **`totalAmount`** (항상) — 세금 포함 총액. **정산·결제 금액의 기준은 어디까지나 총액입니다.** 총액은
  **요청 숙박일 기준**으로 산정합니다 — 공급사가 요청 기간 밖 날짜를 함께 반환하면 어댑터가 그 날짜를
  제외하고 합산해(가용성 판정이 기간 밖 날짜를 무시하는 것과 같은 기준), 총액이 요청 박수와 어긋나지
  않게 합니다.
...
판정 결과는 세 가지지만 — "모르는 0"과 "확실한 0"은 다릅니다 — **미확정은 표준 모델의 상태가
아닙니다.** 판정(`AvailabilityPolicy.judge`)이 null 을 돌려주면 정규화가 그 상품을 제외하므로(미매핑
제외와 같은 층위), 표준 모델의 `Availability` 는 확정 숫자 하나(`availableRooms`, 0 = 확정 매진)만
가집니다.

| 판정 결과 | 조건 | 귀결 |
|---|---|---|
| 예약 가능 | 병목 최소값 ≥ 1 | `Availability(n)` — 잔여 수와 함께 노출 |
| 확정 매진 | 모든 날짜 데이터가 있고 최소값 = 0 | `Availability(0)` — **매진 표시와 함께 노출**, 취소로 재고가 생겼을 때 알림을 받는 기능 등 후속 기능의 진입점 |
| 미확정 | 요청 기간의 날짜 누락 | **null — 정규화에서 제외** (표준 모델에 도달하지 않음), 매진이라고 단정하는 것도 거짓이므로 |
...
| `availability` | StayProduct의 확정 가용성 — `availableRooms` 숫자 하나(0 = 확정 매진). 미확정은 정규화에서 제외되어 모델에 없음 | — | — |
