...
- **중간 표준 타입** (supplier 패키지) — 공급사 코드를 그대로 쓰되 형식은 통일된 타입 두 벌: 숙소
  목록용(코드·이름·정원)과 재고·요금용(코드 + gross 총액 + 날짜별 잔여 + 조식). 원시
  DTO는 `supplier/a`·`supplier/b` 안에 갇혀 이 타입으로만 나옵니다.
- **정규화** (search 유스케이스의 조립 단계) — 중간 타입의 공급사 코드를 내부 id로 치환하고, 가용성
  판정(병목 최소값·엄격 — 미확정은 제외)과 평균 1박가(내림) 계산을 거쳐 `StayProduct`로 조립합니다. 별도 컴포넌트를
  두지 않습니다 — 치환·미매핑 스킵은 검색 1건 동안 미리 로드한 매핑 조회(lookup)가, 판정·요금 규칙은
  domain의 순수 함수가 맡으므로, 남는 조립은 유스케이스 안의 단순 매핑입니다. 매핑에 없는 코드의
  상품(숙소 목록에 없던 상품이 재고 응답에 나타난 경우)은 내부 id를 만들 수 없으므로 치환 단계에서 그
  상품만 버리고 경고 로그를 남깁니다 — 읽기 경로인 검색에서 즉석으로 매핑을 만들지 않으며, 동기화(수동
  트리거 포함)가 매핑을 채우면 다음 검색부터 자연히 나타납니다.
